"use strict";
let primoSelettore = document.getElementById('regioni');
fetch('regioni.json')
...
//console.log(regione)
let secondoSelettore = document.getElementById('province');
fetch('province.json')
    .then(response => response.json())
    .then(province => {
    //console.table(province)
    primoSelettore.addEventListener('click', function () {
        let regione = primoSelettore.value;
        secondoSelettore.querySelectorAll('option').forEach((c) => {
            c.remove();
        });
        for (var i = 0; i < province.length; i++) {
            ...
            if (province[i].prov_reg == regione) {
                ...
                let primaImg = document.getElementById('imgReg');
    ...
    secondoSelettore.addEventListener('click', function () {
        ...
        let selezione = province.find((prov) => prov.prov_nome == provinciaSelezionata);
        //console.log(selezione)
        let secondaImg = document.getElementById('imgPro');
